...
# Configure Gemini with your API key (ensure the GENAI_API_KEY env variable is set)
...
def get_existing_code():
    ...
    skip_files = {"update_word.py", ".gitignore"}
...
def chunk_text(text, chunk_size=5000):
    """Chunk a large text into pieces of up to chunk_size characters."""
    return [text[i:i+chunk_size] for i in range(0, len(text), chunk_size)]

def chunk_code(text, chunk_size=5000):
    """
    Chunk the code using markers (e.g., '# File:') as boundaries.
    If a chunk is still too large, further break it into smaller pieces.
    """
...
def create_final_prompt(doc_chunks, code_chunks, base_prompt):
    """
    Append each chunk of documentation and code to the base prompt.
    This creates a comprehensive prompt that includes your detailed instructions plus the project data.
    """
    prompt = base_prompt + "\n\n"
...
def generate_documentation(prompt):
    """
    Use Gemini's text generation to generate the comprehensive documentation.
    Adjust the model and parameters as needed.
    """
    try:
